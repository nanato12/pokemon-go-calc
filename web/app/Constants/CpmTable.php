...
final class CpmTable
{
     ...
     * 指定レベルのCPM値を取得.
     *
     * @throws InvalidArgumentException 無効なレベルの場合
     */
    public static function get(float $level): float
    {
        $key = self::levelKey($level);

        if (!isset(self::TABLE[$key])) {
            throw new InvalidArgumentException(
                sprintf('無効なレベル: %s. %.1f〜%.1fの0.5刻みで指定してください.', $key, self::MIN_LEVEL, self::MAX_LEVEL)
            );
        }
    ...
    public static function allLevels(): array
    {
        ...
        for ($level = self::MIN_LEVEL; $level <= self::MAX_LEVEL; $level += 0.5) {
            $levels[] = $level;
        }
     ...
     * float を文字列キーに変換.
     */
    private static function levelKey(float $level): string
    {
        // 0.5刻みを扱うため、小数点1桁の文字列にする
